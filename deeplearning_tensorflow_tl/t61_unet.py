...
def unet(x, convs=4, base_filters=64, name=None):
    ...
    with tf.variable_scope(name):
        sentiment, stack = encode(x, convs, base_filters)
        return decode(sentiment, stack)
...
def decode(sentiment, stack):
    stack = reversed(stack)
    filters = sentiment.shape[-1].value
    y = sentiment
