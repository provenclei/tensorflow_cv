...
class Config:
    ...
    def get_sub_tensor(self, id):
        '''
        为每个GPU配置的张量
        :param id: gpu index
        :return: the sub tensors which has the property 'inputs'
        '''
        raise Exception('the get_sub_tensord() is not define')
...
class Tensors:
    ...
    '''
    def __init__(self, lr, config: Config):
        self.config = config
        # 输入张量集合
        self.sub_ts = []  # [gpus]
        # 可训练张量，均需要设置 scope
        ...
        with tf.variable_scope(config.get_name()) as scope:
            for i in range(config.gpus):
                with tf.device('/gpu:%d' % i):
                    self.sub_ts.append(config.get_sub_tensor(i))
...
class App:
    ...
    def get_feed_dict(self, ds):
        ...
        for i in range(self.config.gpus):
            ...
            for tensor, value in zip(self.ts.sub_ts[1].inputs, values):
                result[tensor] = value
        return result
