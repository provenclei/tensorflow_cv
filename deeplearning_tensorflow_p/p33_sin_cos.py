...
class Sample:
    ...
    def __init__(self, samples):
        ...
        self.ys = np.sin(self.xs), np.cos(self.xs), np.square(self.xs)   # [-1, 2]
        # plt.plot(xs, ys)
        # plt.show()
...
class Tensors:
    ...
    def __init__(self, config: Config):
        ...
        self.y = tf.placeholder(tf.float32, [3, None], 'y')  # [2, -1]

        x = tf.reshape(self.x, [-1, 1])
        x = tf.layers.dense(x, config.hidden_units, tf.nn.relu)
        self.y_predict = tf.layers.dense(x, 3)  # [-1, 2]

        y = tf.transpose(self.y)  # [-1, 2]
